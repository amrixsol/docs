...
### 🔔 Real-Time On-Ramp Telegram Alerts

Bridge on-ramp events now send instant Telegram notifications when funds are received or crypto is delivered. Each alert includes the user, amount, payment rail, and a full fee breakdown showing the developer fee, Bridge cost, and net revenue.
...
Fixed a bug where card spending limits were being sent to our card issuer in the wrong unit (dollars instead of cents), which could result in incorrect transaction limits. Monthly spending display in the app has also been corrected.
...
The daily Telegram digest now includes comprehensive revenue metrics: on-ramp and off-ramp volume with fee breakdowns, swap volume, card transaction revenue, KARMA pool LP fees, cashback costs, and total profit calculations. LP revenue from the Meteora DLMM pool is tracked automatically.
...
Deployed a fully automated treasury pipeline. Operational wallets (gas sponsor, card fee, rewards) are automatically topped up from the Believe fee wallet when balances run low. KARMA token liquidation is handled via Meteora DLMM LP positions, and excess SOL from the LP wallet is swept back into the pipeline. Health monitoring crons track all wallet balances and alert on anomalies.
...
On-ramp developer fees have been adjusted to 1% globally, with a 4% rate for users in Argentina to account for higher local payment processing costs. These fees are clearly communicated during the deposit flow.
...
Migrated the swap engine from Jupiter Metis to the new Jupiter Ultra API, delivering faster and more reliable quotes. For tokens that Ultra doesn't support (such as newer meme coins), the system automatically falls back to Jupiter V6 with gasless execution—Karma's treasury pays the gas fee on behalf of the user. A dynamic fee structure ensures a minimum fee of $0.45 per swap, with the standard rate set at 0.5%.
...
### 🤖 Internal CS Agent API

Added a narrow internal API that allows our customer support agent to update specific KYC profile fields on behalf of users. This reduces manual database operations and speeds up support resolution for users who need profile corrections.
...
Cards that have been inactive for an extended period are now automatically deactivated with a push notification nudging the user to reactivate. The card lock mechanism has been upgraded to use Rain's current PATCH API instead of the legacy freeze endpoint.
...
### 💰 On-Chain Fee Collection for Bill Pay

Off-ramp fees for bill pay transfers are now collected on-chain instead of through Bridge's developer fee mechanism, giving us more control and transparency over the fee structure.
